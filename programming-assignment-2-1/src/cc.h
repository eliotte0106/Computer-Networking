...
class Vegas : public CCC {
  public:
    void init() {
        ...
        m_dBeta = 110.0 * (1024.0);
        m_dLinearIncreaseFactor = 9.0 / (1024.0);
    ...
    void onACK(int32_t ack) {
        ...
        //    size by (m_dCWndSize * linear factor). If the difference is above
        //    your beta factor, decrease the window size by (m_dCWndSize * linear
        ...
        double CWND = m_dCWndSize * m_iMSS;
        double expectedThroughput = (CWND * 1000) / m_dBaseRTT;
        double inFlight = (m_iSndCurrSeqNo - ack) * m_iMSS; 
        double actualThroughput = (inFlight * 1000) / m_dCurrentRTT;
        double diff = expectedThroughput - actualThroughput;

        if (diff < m_dAlpha) {
            m_dCWndSize += m_dCWndSize * m_dLinearIncreaseFactor;
